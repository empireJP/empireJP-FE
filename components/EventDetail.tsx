...
/**
 * The whole event page, shared by the public `/events/[slug]` route and the
 * share-link `/e/[token]` route.
 *
 * Extracted rather than duplicated: the two routes differ only in how they
 * resolve the event and what they tell crawlers, and a copy of ~240 lines of
 * layout would drift within a release.
 *
 * `shareToken` is present only on the private route. It has to travel with the
 * user into checkout, because the API refuses a private event's slug — without
 * it the checkout re-fetch 404s and the buyer lands on an empty cart straight
 * after clicking Get Tickets.
 */
export function EventDetail({
  ...
  const isRsvp = event.registrationMode === "RSVP";
      ...
      <div className="grid gap-8 md:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)]">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {/* Says plainly that this page isn't public, so someone who was
                sent the link knows not to repost it. */}
            {event.private && (
              <span className="inline-flex items-center gap-1 rounded-full bg-surface-3 px-3 py-1 text-xs font-semibold text-muted">
              ...
              <div className="flex items-center justify-between border-b border-line bg-surface-2 px-5 py-3">
                ...
                <span className="text-sm text-muted">
                  {soldOut
                    ...
                    : from === 0
                    ? "Free"
                    : `from ${money(from!, event.currency)}`}
                </span>
              </div>
                ...
                {event.tiers.map((t) => (
                  <div key={t.id} className="flex items-start justify-between gap-4 px-5 py-3.5">
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <p
                        className={`text-xs ${
                          !t.soldOut && t.available <= 24 ? "text-warning" : "text-faint"
                        }`}
                      >
                        {t.soldOut
                          ? "Sold out"
